...
def analyze_complete_comparison_data():
    ...
    if 0 in env_data:
        ...
        print(f"    Roll: 均值={np.degrees(np.mean(diff_eta[:, 3])):.6f}°, 标准差={np.degrees(np.std(diff_eta[:, 3])):.6f}°, 最大值={np.degrees(np.max(np.abs(diff_eta[:, 3]))):.6f}°")
        print(f"    Pitch: 均值={np.degrees(np.mean(diff_eta[:, 4])):.6f}°, 标准差={np.degrees(np.std(diff_eta[:, 4])):.6f}°, 最大值={np.degrees(np.max(np.abs(diff_eta[:, 4]))):.6f}°")
        print(f"    Yaw: 均值={np.degrees(np.mean(diff_eta[:, 5])):.6f}°, 标准差={np.degrees(np.std(diff_eta[:, 5])):.6f}°, 最大值={np.degrees(np.max(np.abs(diff_eta[:, 5]))):.6f}°")
        ...
        print(f"    Roll: 均值={np.degrees(np.mean(diff_nu[:, 3])):.6f}°/s, 标准差={np.degrees(np.std(diff_nu[:, 3])):.6f}°/s, 最大值={np.degrees(np.max(np.abs(diff_nu[:, 3]))):.6f}°/s")
        print(f"    Pitch: 均值={np.degrees(np.mean(diff_nu[:, 4])):.6f}°/s, 标准差={np.degrees(np.std(diff_nu[:, 4])):.6f}°/s, 最大值={np.degrees(np.max(np.abs(diff_nu[:, 4]))):.6f}°/s")
        print(f"    Yaw: 均值={np.degrees(np.mean(diff_nu[:, 5])):.6f}°/s, 标准差={np.degrees(np.std(diff_nu[:, 5])):.6f}°/s, 最大值={np.degrees(np.max(np.abs(diff_nu[:, 5]))):.6f}°/s")
...
def plot_complete_comparison_results(env_data):
    ...
    # 位置对比 - 后3个自由度（角度）
    plt.subplot(2, 3, 4)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_eta'][:, 3]), 'r-', label='IsaacLab Roll Angle', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_eta'][:, 3]), 'b--', label='Calculated Roll Angle', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    plt.subplot(2, 3, 5)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_eta'][:, 4]), 'r-', label='IsaacLab Pitch Angle', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_eta'][:, 4]), 'b--', label='Calculated Pitch Angle', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    plt.subplot(2, 3, 6)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_eta'][:, 5]), 'r-', label='IsaacLab Yaw Angle', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_eta'][:, 5]), 'b--', label='Calculated Yaw Angle', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    # 速度对比 - 后3个自由度（角速度）
    plt.subplot(2, 3, 4)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_nu'][:, 3]), 'r-', label='IsaacLab Roll Rate', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_nu'][:, 3]), 'b--', label='Calculated Roll Rate', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    plt.subplot(2, 3, 5)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_nu'][:, 4]), 'r-', label='IsaacLab Pitch Rate', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_nu'][:, 4]), 'b--', label='Calculated Pitch Rate', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    plt.subplot(2, 3, 6)
    plt.plot(data['time_steps'], np.degrees(data['isaaclab_nu'][:, 5]), 'r-', label='IsaacLab Yaw Rate', linewidth=1.5, alpha=0.8)
    plt.plot(data['time_steps'], np.degrees(data['calculated_nu'][:, 5]), 'b--', label='Calculated Yaw Rate', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    ...
    plt.subplot(2, 3, 3)
    plt.plot(data['time_steps'], np.degrees(diff_eta[:, 5]), 'g-', label='Yaw Angle Difference', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    plt.ylabel('Yaw Angle Difference (deg)')
    ...
    plt.subplot(2, 3, 6)
    plt.plot(data['time_steps'], np.degrees(diff_nu[:, 5]), 'orange', label='Yaw Rate Difference', linewidth=1.5, alpha=0.8)
    plt.xlabel('Time Steps')
    plt.ylabel('Yaw Rate Difference (deg/s)')
